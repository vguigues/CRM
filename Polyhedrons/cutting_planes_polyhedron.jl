...
-  ϵ::Real:             precision parameter
"""
function update_a3pm!(x::AbstractVector, p::AbstractMatrix, iter::Int, x0::AbstractVector)
	m, n = size(p)
...
function update_sc_crm_over!(x::AbstractVector, A::UInt, B::UInt, y::AbstractMatrix, Q::AbstractArray,
	env::Gurobi.Env, approx_epsilon::Real)
	z_bar = Z(x, A, B, y, Q, env)
...
function update_sc_crm_over_back!(x::AbstractVector, A::UInt, B::UInt, y::AbstractMatrix, Q::AbstractArray,
	env::Gurobi.Env, max_backtracking::UInt, epsilonBack::Real, epsilonStop::Real, beta::Real)

	found_next = false
	nb_iter = 1
	xcrm=update_sc_crm!(x, A, B, y, Q, env)
	violation_crm=max_violation(xcrm, y, Q)
	if violation_crm <= epsilonStop
		return xcrm
	else
		while nb_iter<=max_backtracking && !found_next
			x_over=xcrm+epsilonBack*(xcrm-x)
			violation_crm_over=max_violation(x_over, y, Q)
...
function update_sc_crm_under_back!(x::AbstractVector, A::UInt, B::UInt, y::AbstractMatrix, Q::AbstractArray,
	...
	if violation_crm <= epsilonStop
		...
		while nb_iter<=max_backtracking && !found_next
			...
			violation_crm_under=max_violation(x_under, y, Q)
...
function solve_a3pm(x0::AbstractVector, y::AbstractMatrix, Q::AbstractArray,
...
function solve_alt_proj(x0::AbstractVector, y::AbstractMatrix, Q::AbstractArray,
	...
	t_start = time()
	while (norm(x - target) > epsilon)
		update_alt_proj!(x, y, Q, env, iter, x0)
		current_violation = max_violation(x, y, Q)
		push!(violation, current_violation)
		iter += 1
	end
...
- ϵ::Real               precision parameter
"""
function solve_cimmino(x0::AbstractVector, y::AbstractMatrix, Q::AbstractArray, env::Gurobi.Env,
	epsilon::Real, target::AbstractVector, mt::Bool = false)
	x = copy(x0)
	iter = 1
	violation = []
	...
	while (norm(x - target) > epsilon)
		if mt
			update_cimmino_mt!(x, y, Q, env, iter, x0)
		else
			update_cimmino!(x, y, Q, env, iter, x0)
		end
		# push!(points_x, x[1])
		# push!(points_y, x[2])
		current_violation = max_violation(x, y, Q)
...
function solve_sc_crm_under(x0::AbstractVector, y::AbstractMatrix, Q::AbstractArray,
	...
	while (norm(x - target) > epsilon)
		A::UInt = (iter % k) + 1
		B::UInt = A == k ? 1 : A + 1
		update_sc_crm_under!(x, A, B, y, Q, env, approx_epsilon(iter))
		# push!(points_x, x[1])
		# push!(points_y, x[2])
		iter += 1
		current_violation = max_violation(x, y, Q)
...
function solve_sc_crm_over(x0::AbstractVector, y::AbstractMatrix, Q::AbstractArray,
	...
	while (norm(x - target) > epsilon)
		A::UInt = (iter % k) + 1
		B::UInt = A == k ? 1 : A + 1
		update_sc_crm_over!(x, A, B, y, Q, env, approx_epsilon(iter))
		# push!(points_x, x[1])
		# push!(points_y, x[2])
		iter += 1
		current_violation = max_violation(x, y, Q)
...
function solve_sc_crm_back_over(x0::AbstractVector, y::AbstractMatrix, Q::AbstractArray, max_iter::UInt, env::Gurobi.Env, ϵ::Real, max_backtracking::UInt, epsilonBack::Real, epsilonStop::Real, beta::Real)
	x = copy(x0)
	k, _, _ = size(Q)
	iter = 1
	violation = []
	...
	while (norm(x - target) > epsilon)
		A::UInt = (iter % k) + 1
		B::UInt = A == k ? 1 : A + 1
		update_sc_crm_over_back!(x, A, B, y, Q, env, max_backtracking, epsilonBack, epsilonStop, beta)
		iter += 1
		current_violation = max_violation(x, y, Q)
		push!(violation, current_violation)
	end
...
function solve_sc_crm_back_under(x0::AbstractVector, y::AbstractMatrix, Q::AbstractArray, max_iter::UInt, env::Gurobi.Env, ϵ::Real, max_backtracking::UInt, epsilonBack::Real, epsilonStop::Real, beta::Real)
	x = copy(x0)
	k, _, _ = size(Q)
	iter = 1
	violation = []
	...
	while (norm(x - target) > epsilon)
		A::UInt = (iter % k) + 1
		B::UInt = A == k ? 1 : A + 1
		update_sc_crm_under_back!(x, A, B, y, Q, env, max_backtracking, epsilonBack, epsilonStop, beta)
		# push!(points_x, x[1])
		# push!(points_y, x[2])
		iter += 1
		current_violation = max_violation(x, y, Q)
...
function test_cutting_plane2(n::UInt)
	...
	x_cimmino, violation_cimmino, iter_cimmino, time_cimmino = solve_cimmino(x0, y, Q, env, ϵ, target, false)
	...
	# x_crm, violation_crm, iter_crm, time_crm = solve_crm(x0, y, Q, env, ϵ, target)
...
function test_cutting_plane(k::UInt, n::UInt, m::UInt, max_iter::UInt, alpha::Real)
	...
	# x_a3pm, violation_a3pm, iter_a3pm, time_a3pm = solve_a3pm(x0, y, Q, ϵ, target, true)
	# time_a3pm = measure_fast_method_time(() -> solve_a3pm_halpern(x0, y, Q, k, max_iter, env, ϵ, target), time_a3pm)
	# @show violation_a3pm[end], time_a3pm, iter_a3pm

	# x_3pm, violation_3pm, iter_3pm, time_3pm = solve_3pm(x0, y, Q, max_iter, env, ϵ, target)
	...
	x_sccrm_under, violation_sccrm_under, iter_sccrm_under, time_sccrm_under = solve_sc_crm_under(x0, y, Q, env, ϵ, target)
	println("\tCRM Under: $time_sccrm_under s, iter = $iter_sccrm_under, violation = $(violation_sccrm_under[end])")
